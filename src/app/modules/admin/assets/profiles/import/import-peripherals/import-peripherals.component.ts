...
@Component({
  selector: 'app-import-peripherals',
  templateUrl: './import-peripherals.component.html',
  styleUrls: ['./import-peripherals.component.scss']
})
export class ImportPeripheralsComponent implements OnInit {
  data: any[] = [];
  ...
  uploadData() {
    ...
    const formattedData = this.data.slice(1).map((row: any[]) => ({
      ...
      peripheral_type: String(row[2] || "N/A"),
      brand: String(this.excelDateToString(row[3])) || "N/A",
      model: String(row[4] || "N/A"),
      size: String(row[5] || "N/A"),
      color: String(row[6] || "N/A"),
      li_description: String(row[7] || "N/A"),     
      serial_no: String(row[8] || "N/A"),  
      assigned: String(row[9] || "Not Assigned"),
      status: String(row[10] || "Active"),  
      history: String(row[11] || " "),  
    }));
