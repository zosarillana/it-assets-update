...
@Component({
    selector: 'app-modal-create-pc-card',
    templateUrl: './modal-create-pc-card.component.html',
    styleUrls: ['./modal-create-pc-card.component.scss'],
    animations: [
        trigger('fadeInOut', [
            transition(':enter', [
                style({ opacity: 0 }),
...
export class ModalCreatePcCardComponent implements OnInit {
    ...
    submitForm() {
        ...
        if (this.horizontalStepperForm.valid) {
            ...
            this.cardService.CreateCardData(cardData).subscribe({
                ...
                error: (error) => {
                    console.error('Error creating PC Card', error);
                    this.alertService.triggerError('Error creating PC Card');
